# 🌾 AgriAI Web Application

AI-powered agriculture platform connecting farmers and buyers for crop trading, price forecasting, and data-driven insights using React, Node.js, and MongoDB.

---

## 🧩 Project Overview

 Building a full-stack web application called **AgriAI**, where:

👨‍🌾 Farmers can register, log in, and post their crops for sale (with quantity, price, image, and description).

🛒 Buyers can register, browse crops, and purchase.

📈 A Price Prediction AI microservice (Python + Flask) predicts future crop prices.

🧮 The app displays graphs of farmer/buyer history and expected prices using charts.

💾 Database: **MongoDB**

🧱 Backend: **Node.js + Express + JWT Auth + Mongoose**

🖥 Frontend: **React + Tailwind + Axios + Recharts**

*(and keep the rest of your detailed content exactly as you had it before)*

